...
_ANIM_DURATION = 5.0
_LAYOUTS = ('std140', 'std430', 'uniform')
...
def _get_render(cfg, quad, fields, block_definition, color_definition, block_fields, color_fields, layout, debug_positions=False):
    ...
    program = ngl.Program(vertex=header + _FIELDS_VERT, fragment=fragment)
...
def _get_debug_positions(spec, field_id):
    fields_info, block_fields, color_fields, block_definition, color_definition = _get_random_block_info(spec) #seed, layout)
    fields = _get_field(spec, field_id)
    return _get_debug_positions_from_fields(fields)



def _get_random_block_info(spec, seed=0, layout=_LAYOUTS[0], color_tint=True):
    ...
    if layout == 'uniform':
        ...
        print 'block fields:', block_fields
...
def _get_render_with_legend(cfg, fields, area, title, block_definition, color_definition, block_fields, color_fields, layout):
    ...
    render = _get_render(cfg, quad, fields, block_definition, color_definition, block_fields, color_fields, layout)
