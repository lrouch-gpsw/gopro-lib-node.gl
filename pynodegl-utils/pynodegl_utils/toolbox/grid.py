...
def _grid(scenes, duration=None, overlap_time=None):
...
def grid_queue(scenes, duration, overlap_time):
    return _grid(scenes, duration, overlap_time)
...
if __name__ == '__main__':
    ...
    for n in range(1, N*N+1):
        grid = AutoGrid(n)
        buf = [['.'] * grid.nb_cols for i in range(grid.nb_rows)]
        for i, x, y in grid:
            buf[y][x] = 'x'
        print '#%d (cols:%d rows:%d)' % (n, grid.nb_cols, grid.nb_rows)
